...
func run(
    ...
    if let dump, let directory = ProcessInfo.processInfo.environment["AFTERMATH_DUMP"] {
...
@MainActor
func run(_ key: MishapKey, _ route: Route, frames: Int? = nil, reading: Set<Int>? = nil) throws -> [Int: Picture] {
    let mishap = Mishaps.named(key)
    let frames = frames ?? mishap.frames
    return try run(mishap.make(route), frames: frames, reading: reading ?? Set(1...frames), dump: "\(key)-\(route)")
}
...
final class Scene: Sketch {
    ...
    let body: (Scene) -> Void
    init(_ settings: SketchSettings = Mishaps.settings, _ body: @escaping (Scene) -> Void) {
...
/// (`writePNG` = 表示に符号化した絵) で、置き場は `<PROBES_SHOTS>/Aftermath/<name>-<frame>.png`。
/// 変数が無ければ何もしない。
@MainActor
func shoot(_ target: RenderTarget, name: String, frame: Int) throws {
    ...
    let directory = URL(fileURLWithPath: root).appendingPathComponent("Aftermath", isDirectory: true)
...
func fingerprint(_ pixels: PixelBuffer, name: String, frame: Int) throws {
    ...
    let url = URL(fileURLWithPath: directory).appendingPathComponent("Aftermath-\(getpid()).tsv")
