...
- **指紋** — 描いた絵のハッシュ。**同じフレーム番号からはバイト単位で同じ絵が出る**
  (mokume ADR-0001 原則 2) ので、実行をまたいで食い違えば、許容誤差の内のずれでも破れである
    ...
    python3 scripts/stress.py                              # Probe Drift Routine Reach Aftermath Imprint Lattice Weave を全条件で
...
RULERS = ["Probe", "Drift", "Routine", "Reach", "Aftermath", "Imprint", "Lattice", "Weave"]
