...
@Suite(.serialized) struct SoakTests {
    ...
    func unclosedShape() async throws {
        ...
        withKnownIssue("mokume#NNNN: 開いた形の印がフレームの境目で下りず、vertex() が点を積み続ける") {
    ...
    func offFrameGraphics() async throws {
        ...
        withKnownIssue("mokume#NNNN: 図形の口がフレームの外を見ずに溜め場へ積み、捨てる契機が来ない") {
    ...
    func modelSequence() async throws {
        ...
        withKnownIssue("mokume#NNNN: loadModel の控えに上限も追い出しも無い") {
    ...
    func headlessAdvance() async throws {
        ...
        withKnownIssue("mokume#NNNN: GPU の完了の後始末を main actor の Task に積むので、譲らないと走らない") {
    ...
    func polygonFill() async throws {
        ...
        withKnownIssue("mokume#NNNN: 耳を探すたびに頭から走査し、耳の判定で全頂点を見る (頂点数の二乗)") {
            #expect(
    ...
    func solidStroke() async throws {
        ...
        withKnownIssue("mokume#NNNN: 稜線の帯と頂点ごとの円板を、立体ごと・毎フレーム CPU で組み直す") {
    ...
    func hugeTextSize() async {
        ...
        await withKnownIssue("mokume#NNNN: 字形の外接矩形を Int へ直すところで、大きさの上限を見ていない") {
    ...
    func textOutline() async {
        ...
        await withKnownIssue("mokume#NNNN: 曲線を割る数を Int((rough / 2).rounded(.up)) で作り、rough が NaN / inf になる") {
    ...
    func createShapeDiscard() async {
        ...
        await withKnownIssue("mokume#NNNN: 溜め場を空にした後で、入口で覚えた開始位置から切り出す") {
    ...
    func makeNumbersHuge() async {
        ...
        await withKnownIssue("mokume#NNNN: 上限を検める前のバイト数の掛け算があふれる") {
    ...
    func displayImageOutside() async {
        ...
        await withKnownIssue("mokume#NNNN: DisplayImage の添字が precondition で止まる (PixelBuffer は #1436 で透明を返す)") {
