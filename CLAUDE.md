...
mokume を外から測る物差し (Atlas・Probe・Drift・Routine・Reach・Soak・Aftermath・Imprint・Lattice) を置く。何を置くか・並べ方・走らせ方は
...
- **`swift test` は CI では回らない。** 画素を読むので Metal が要り、GitHub の Linux の
  runner には無い。CI (`pr-policy`) が見るのは PR タイトルだけなので、Probe・Drift・Routine・Soak・Aftermath・Imprint・Lattice を
...
- **画素を見る 6 本 (Probe・Drift・Routine・Reach・Aftermath・Lattice) の `Stage.swift` で絵を読んだら、`fingerprint` を通す。**
